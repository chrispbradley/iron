...
999 ERRORSEXITS("PARMETIS_PARTKWAY",ERR,ERROR)
  ...
  SUBROUTINE PARMETIS_PARTMESHKWAY( ELEMENT_DISTANCE, ELEMENT_PTR, ELEMENT_INDEX, minNumberXi, &
                                  & NUMBER_PARTS, OPTIONS, NUMBER_EDGES_CUT, PARTITION, &
                                  & COMMUNICATOR, ERR, ERROR, * )

    !Argument Variables
    INTEGER(INTG), INTENT(IN) :: ELEMENT_DISTANCE(:)
    INTEGER(INTG), INTENT(IN) :: ELEMENT_PTR(:)
    INTEGER(INTG), INTENT(IN) :: ELEMENT_INDEX(:)
    INTEGER(INTG), INTENT(IN) :: minNumberXi
    INTEGER(INTG), INTENT(IN) :: NUMBER_PARTS
    INTEGER(INTG), INTENT(IN) :: OPTIONS(:)
    INTEGER(INTG), INTENT(OUT) :: NUMBER_EDGES_CUT
    INTEGER(INTG), INTENT(OUT) :: PARTITION(:)
    ...
    !Local Variables
    integer(INTG) :: NUMBER_COMMON_NODES
    integer(INTG) :: WEIGHT_FLAG=0
    integer(INTG) :: NUM_FLAG=0
    integer(INTG) :: NCON=1
    integer(INTG), dimension(1) :: ELEMENT_WEIGHT
    real(DP),      dimension(1) :: UB_VEC
    real(DP),      dimension(:), allocatable :: TP_WEIGHTS
    ...
    IF( minNumberXi==1 ) THEN
       NUMBER_COMMON_NODES=1
    ELSE
       NUMBER_COMMON_NODES=2
    ENDIF

    allocate( TP_WEIGHTS(NUMBER_PARTS) )
    TP_WEIGHTS = 1.0_DP/REAL(NUMBER_PARTS,DP)

    CALL ParMETIS_V3_PartMeshKway( ELEMENT_DISTANCE, ELEMENT_PTR, ELEMENT_INDEX, ELEMENT_WEIGHT, WEIGHT_FLAG, NUM_FLAG, NCON, &
                                 & NUMBER_COMMON_NODES, NUMBER_PARTS, TP_WEIGHTS, UB_VEC, OPTIONS, NUMBER_EDGES_CUT, &
