...
998 ERRORSEXITS("DISTRIBUTED_VECTOR_CMISS_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE DISTRIBUTED_VECTOR_CMISS_CREATE_FINISH(CMISS_VECTOR,ERR,ERROR,*)
    ...
    LOGICAL              :: FOUND
    TYPE(VARYING_STRING) :: DUMMY_ERROR, LOCAL_ERROR
    INTEGER(INTG)        :: domain_idx, domain_idx2, domain_no, DUMMY_ERR, subdomain
  ...
  ! pointer checks
    IF( .not.ASSOCIATED(CMISS_VECTOR) ) call FlagError( "CMISS vector is not associated", ERR, ERROR, *999 )
    IF ( .not.ASSOCIATED(CMISS_VECTOR%DISTRIBUTED_VECTOR) ) &
       & CALL FlagError( "CMISS vector distributed vector is not associated", ERR, ERROR ,*999 )
    IF ( .not.ASSOCIATED(CMISS_VECTOR%DISTRIBUTED_VECTOR%DOMAIN_MAPPING) ) &
  ...
  ! set some convenient pointers
    ...
    MAPPING => DISTRIBUTED_VECTOR%DOMAIN_MAPPING
    if ( .not.allocated( MAPPING%ADJACENT_DOMAINS_PTR ) ) &
      & call FlagError( "domain mapping adjacent domains ptr not allocated", ERR, ERROR, *999 )

  ! read vector size
    CMISS_VECTOR%DATA_SIZE = CMISS_VECTOR%N

  ! determine the kind of the vector
    ...
    IF ( MAPPING%NUMBER_OF_DOMAINS==1 ) THEN
       DISTRIBUTED_DATA_ID = DISTRIBUTED_DATA_ID + 1
    ELSE
       DISTRIBUTED_DATA_ID = DISTRIBUTED_DATA_ID + MAPPING%ADJACENT_DOMAINS_PTR(MAPPING%NUMBER_OF_DOMAINS)
    ENDIF

    IF ( MAPPING%NUMBER_OF_ADJACENT_DOMAINS>0 ) THEN
       ...
       IF (DISTRIBUTED_VECTOR%GHOSTING_TYPE==DISTRIBUTED_MATRIX_VECTOR_INCLUDE_GHOSTS_TYPE) THEN
          ALLOCATE(CMISS_VECTOR%TRANSFERS(MAPPING%NUMBER_OF_ADJACENT_DOMAINS),STAT=ERR)
          ...
          DO domain_idx = 1,MAPPING%NUMBER_OF_ADJACENT_DOMAINS
             ...
             DO domain_idx2=MAPPING%ADJACENT_DOMAINS_PTR(domain_no),MAPPING%ADJACENT_DOMAINS_PTR(domain_no+1)-1
                IF(MAPPING%ADJACENT_DOMAINS_LIST(domain_idx2)==subdomain) THEN
                   FOUND=.TRUE.
                   EXIT
                ENDIF
             ENDDO !domain_idx2
             ...
             SELECT CASE( DISTRIBUTED_VECTOR%DATA_TYPE )
                CASE( DISTRIBUTED_MATRIX_VECTOR_INTG_TYPE )
                   ...
                   IF(ERR/=0) CALL FlagError( "Could not allocate distributed vector send integer transfer buffer", &
                                            & ERR, ERROR, *999 )
...
998 ERRORSEXITS("DISTRIBUTED_VECTOR_CMISS_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE DISTRIBUTED_VECTOR_CREATE_FINISH(DISTRIBUTED_VECTOR,ERR,ERROR,*)
    ...
    IF ( .not.ASSOCIATED(DISTRIBUTED_VECTOR) ) return
    IF ( DISTRIBUTED_VECTOR%VECTOR_FINISHED ) call FlagError( "Distributed vector already finished", ERR, ERROR, *999 )

    SELECT CASE( DISTRIBUTED_VECTOR%LIBRARY_TYPE )
       CASE( DISTRIBUTED_MATRIX_VECTOR_CMISS_TYPE )
          IF ( .not.ASSOCIATED(DISTRIBUTED_VECTOR%CMISS) ) &
            & CALL FlagError( "Distributed CMISS vector not associated", ERR, ERROR, *999 )
          CALL DISTRIBUTED_VECTOR_CMISS_CREATE_FINISH( DISTRIBUTED_VECTOR%CMISS, ERR, ERROR, *999 )

       CASE( DISTRIBUTED_MATRIX_VECTOR_PETSC_TYPE )
          IF ( .not.ASSOCIATED(DISTRIBUTED_VECTOR%PETSC) ) &
            & CALL FlagError( "Distributed PETSC vector not associated", ERR, ERROR, *999 )
...
998 ERRORSEXITS("DISTRIBUTED_VECTOR_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE DISTRIBUTED_VECTOR_CREATE_START(DOMAIN_MAPPING,DISTRIBUTED_VECTOR,ERR,ERROR,*)
    ...
    !Local Variables
     ...
     IF (.not.ASSOCIATED(DOMAIN_MAPPING)) CALL FlagError( "Domain mapping not associated", ERR, ERROR, *998 )
     IF (ASSOCIATED(DISTRIBUTED_VECTOR)) CALL FlagError( "Distributed vector already associated", ERR, ERROR, *998 )
